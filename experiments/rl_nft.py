...
import numpy as np
import torch
from diffusiongym import Reward, Sample
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project_name", type=str, default="ground_up")
    parser.add_argument("--run_name", type=str, default="hv_nft")
    parser.add_argument("--seed", type=int, default=5)

    # algorithm and problem
    parser.add_argument("--reward", type=str, choices=["sa", "qed"], default="qed")
...
def evaluate(
    samples: list[Sample], reward: Reward
...
def main(config: Namespace) -> None:
    ...
    assert (not config.fulfill_num_samples and not config.only_valids) or (
        config.fulfill_num_samples and config.only_valids
    )
    ...
    for _ in tqdm(range(start_epoch, config.epochs)):
        ...
        for name, cnt, total, mean, p50, p95 in rows:
            print(
                f"{name:30s}  n={cnt:5d}  total={total:8.3f}s  mean={mean * 1e3:7.2f}ms  "
